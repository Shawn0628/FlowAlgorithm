...
public class OptimalFlowNetwork implements FlowNetwork {
	...
	public OptimalFlowNetwork(int p, ArrayList<RegularNode> array,
			double transimitionRange) {
		...
		for (int i = 0; i < V.size() - 2; i = i + 2) {
			addEdge(new FlowEdge(V.get(i), V.get(i + 1), V.get(i).getEnergy(),
					true, false, 0));
		}

		// Add edge between non-DG and t
		for (int i = 0; i < splitNonDGNodeList.size(); i++) {
			addEdge(new FlowEdge(splitNonDGNodeList.get(i),
					V.get(V.size() - 1), splitNonDGNodeList.get(i)
							.getStorageCapacity(), false, false, 0));
		}
	...
	private ArrayList<RegularNode> getSplitNonDGNodeList(int p,

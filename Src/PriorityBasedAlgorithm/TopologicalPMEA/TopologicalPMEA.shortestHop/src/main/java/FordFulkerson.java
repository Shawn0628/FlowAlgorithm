...
public class FordFulkerson implements Cloneable {
    ...
    private double nextTransmissionCost = GenerateGraphController.REVEIVINGENERGY;
    private double minLength = Double.MAX_VALUE;
    
    // max flow in flow network G from s to t
    public FordFulkerson(FlowNetwork G, RegularNode source, RegularNode sink) {
        int s = source.getID();
        int t = sink.getID();
    	value = excess(G, t);
    	this.G = G;
        ...
        for(int i=0; i<G.getSourceNodeList().size(); i++){
        	...
        	for (int v = t; v != s; v = edgeTo[v].other(v)) {
        		...
        		if (v==secondEdge)
        			{transmissionCost = GenerateGraphController.REVEIVINGENERGY;}
        		else if (v==penultEdge)
        			{transmissionCost = nextTransmissionCost;}
        		else 
        			{transmissionCost = nextTransmissionCost + GenerateGraphController.REVEIVINGENERGY;}
    ...
    private boolean hasAugmentingPath(FlowNetwork G, int s, int t) {
      ...
      Queue<Integer> q = new Queue<Integer>();
        ...
        while (!q.isEmpty()) {
            int v = q.dequeue();
            //System.out.println("**************************************");
            for (FlowEdge e : G.getSortedAdjEdge(v)) {
                //System.out.println(e.getLength());
            	int w = e.other(v);
                if( e.isEnergyEdge() == true)
        		{
                    if(isSecondEdge(e,s)){
                    	...
                    	for (FlowEdge e2 : G.adj(w)) {
                    		if(e2.isTransmisionEdge()) minLength = Math.min(minLength, e2.getLength());
                    	}
                    	if (e.residualCapacityTo(w)/(GenerateGraphController.REVEIVINGENERGY + minLength)>=1) {
                ...
                else if( e.isTransmisionEdge() && edgeTo[v].isEnergyEdge()){
                	
	                	if(isSecondEdge(edgeTo[v],s)){
	                    	if (edgeTo[v].residualCapacityTo(v)/e.getLength() >= 1) {
	                            if (!marked[w]) {
	                                edgeTo[w] = e;
	                                marked[w] = true;
	                                q.enqueue(w);
	                		...
	                		if (edgeTo[v].residualCapacityTo(v)/(e.getLength() + GenerateGraphController.REVEIVINGENERGY) >= 1) {
	...
	private boolean isSecondEdge(FlowEdge e, int s) {
		// TODO Auto-generated method stub
		 for (FlowEdge edge : G.adj(e.from())) {
	        	int p = edge.other(e.from());
	        	if (p == s)
	        		 return true;
		}
			return false;
